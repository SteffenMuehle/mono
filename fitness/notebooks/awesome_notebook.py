...
df = pd.merge(
    left=df,
    right=df.groupby(["date", "exercise"])
    .agg(
...
exercise_widget = widgets.Dropdown(
    options=df["exercise"].unique(), value=df["exercise"].unique()[0], description="Exercise:"
)

attribute_widget = widgets.Dropdown(
    options=["volume", "intensity", "repetitions", "num_sets"], value="volume", description="Attribute:"
)
...
g = go.FigureWidget(
    data=[trace],
    layout=go.Layout(title=dict(text="Exercise Data Over Time"), xaxis=dict(title="Date"), yaxis=dict(title="Value")),
)
